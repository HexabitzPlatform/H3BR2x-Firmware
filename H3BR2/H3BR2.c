...
Segment_Codes Digit[2] = {Empty}; //Digit[0]: LSD, Digit[1]: MSD
 char letter[2];
 int32_t Number;
 uint8_t StartSevSeg;
...
{
	 "sevendisplaynumber", /* The command string to type. */
 ...
 */
/*----------------------------------------------------------*/
Module_Status SevenDisplayNumber(uint32_t Number)
{
...
hexacode=hexadigit[0]*16 +hexadigit[1] ;
		...
		if(status == H3BR2_OK)
		{
			sprintf((char* )pcWriteBuffer,(char* )pcOKMessage,hexacode);
...
portBASE_TYPE CLI_SevenDisplayOneNumberhCommand( int8_t *pcWriteBuffer, size_t xWriteBufferLen, const int8_t *pcCommandString ){
	...
	if(status == H3BR2_OK)
	{
		sprintf((char* )pcWriteBuffer,(char* )pcOKMessage,hexadigit);
